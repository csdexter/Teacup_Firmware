...
#ifdef HOST
  #define enable_transmit()      do { WRITE(TX_ENABLE_PIN,1);  UCSR1B &=~_BV(RXEN1); } while(0)
  #define disable_transmit()      do { WRITE(TX_ENABLE_PIN,0);  UCSR1B &= ~(_BV(TXCIE1) | _BV(UDRIE1)); UCSR1B |= _BV(RXEN1); } while(0)
#else
  #define enable_transmit()      do { WRITE(TX_ENABLE_PIN,1);  UCSR0B &= ~_BV(RXEN0); } while(0)
  #define disable_transmit()      do { WRITE(TX_ENABLE_PIN,0);  UCSR0B &= ~(_BV(TXCIE0) | _BV(UDRIE0)); UCSR0B |= _BV(RXEN0); } while(0)
#endif
